...
/*
	...
	Tags: chat, box, ajax, forum, private, avatars, filtering, smilies, secure, antispam, html5, messaging, im, instant message, chatbox
	Author: Jeff Starr
...
function sac_shout_edit() {
	...
	if (!current_user_can('manage_options')) {
		...
		if (isset($_GET['sac_comment_id'])) {
			$wpdb->query($wpdb->prepare("UPDATE " . $table_prefix . "ajax_chat SET text = %s WHERE id = %d", esc_sql($_GET['sac_text']), esc_sql($_GET['sac_comment_id'])));
